...
impl event::EventHandler for MainState {
    fn update(&mut self, ctx: &mut Context) -> GameResult {
        ...
        ggez::timer::sleep(Duration::from_millis(30));
        let game_data = GameData {
            pos_player1: self.player_1_pos.y,
            pos_player2: self.player_2_pos.y,
            pos_ball_x: self.ball_pos.x,
            pos_ball_y: self.ball_pos.y,
            score_player_1: self.player_1_score,
            score_player_2: self.player_2_score,
        };
        if let Err(err) = self.tx.try_send(game_data) {
